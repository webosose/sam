...
void Prelauncher4Base::add_item(AppLaunchingItemPtr item) {
  ...
  auto it = std::find_if(item_queue_.begin(), item_queue_.end(),
  ...
  if (set_prelaunching_stages_4_base(new_item) == false) {
    LOG_ERROR(MSGID_APPLAUNCH_ERR, 2,
              PMLOGKS("app_id", item->app_id().c_str()),
              PMLOGKS("reason", "set_stages_fail"), "err: %s", new_item->err_text().c_str());
...
void Prelauncher4Base::remove_item(const std::string& app_uid) {
  auto it = std::find_if(item_queue_.begin(), item_queue_.end(),
            [&app_uid](AppLaunchingItemPtr it_item) {return (it_item->uid() == app_uid);});
  if (it != item_queue_.end())
...
void Prelauncher4Base::remove_item_from_lscall_request_list(const std::string& uid) {
  auto it = std::find_if(lscall_request_list_.begin(),
      lscall_request_list_.end(),
...
bool Prelauncher4Base::cb_return_lscall_for_bridged_request(LSHandle* handle, LSMessage* lsmsg, void* user_data) {
  pbnjson::JValue jmsg = JUtil::parse(LSMessageGetPayload(lsmsg),
      std::string(""));
  if (jmsg.isNull()) {
    LOG_ERROR(MSGID_APPLAUNCH_ERR, 2,
  ...
  if (g_this->get_item_by_uid(uid) == NULL) {
    LOG_INFO(MSGID_APPLAUNCH_ERR, 3,
             ...
             PMLOGKS("where", "bridged_prelaunching_cb_return"), "launching item is already removed");
    return true;
  }
  ...
  LOG_INFO(MSGID_APPLAUNCH, 2,
           ...
           PMLOGJSON("payload", JUtil::jsonToString(jmsg).c_str()), "received return for just bridge request");
...
bool Prelauncher4Base::cb_return_lscall(LSHandle* handle, LSMessage* lsmsg, void* user_data) {
  ...
  if (g_this->get_item_by_uid(uid) == NULL) {
    LOG_INFO(MSGID_APPLAUNCH_ERR, 3,
             PMLOGKS("uid", uid.c_str()),
             PMLOGKS("reason", "not_valid_prelaunching_item"),
             PMLOGKS("where", "prelaunching_cb_return"), "launching item is already removed");
    return true;
  }
...
void Prelauncher4Base::input_bridged_return(AppLaunchingItemPtr item, const pbnjson::JValue& jmsg) {
  ...
  LOG_INFO(MSGID_APPLAUNCH, 3,
           PMLOGKS("app_id", item->app_id().c_str()),
           PMLOGJSON("jmsg", JUtil::jsonToString(jmsg).c_str()),
...
void Prelauncher4Base::run_stages(AppLaunchingItem4BasePtr prelaunching_item) {
  if (prelaunching_item == NULL) {
    LOG_ERROR(MSGID_APPLAUNCH_ERR, 2,
              PMLOGKS("reason", "null_prelaunching_item"),
              PMLOGKS("where", "run_prelaunching_stages"), "");
    return;
  }
  ...
  while (StageHandlerType4Base::DIRECT_CHECK == stage_item.handler_type) {
    ...
    if (stage_list.empty()) {
      LOG_INFO(MSGID_APPLAUNCH, 1,
               PMLOGKS("app_id", prelaunching_item->app_id().c_str()), "handled all stages");
      finish_prelaunching(prelaunching_item);
      return;
    }
  ...
  if (!stage_item.payload_maker(prelaunching_item, payload)) {
    LOG_ERROR(MSGID_APPLAUNCH_ERR, 1,
              PMLOGKS("stage", "prelaunching"), "run_stage: failed to make payload");
  ...
  if (!LSCallOneReply(AppMgrService::instance().ServiceHandle(),
                      stage_item.uri.c_str(), JUtil::jsonToString(payload).c_str(),
    ...
    LOG_ERROR(MSGID_LSCALL_ERR, 3,
              ...
              PMLOGKS("where", stage_item.uri.c_str()), "err: %s", lserror.message);
    prelaunching_item->set_err_code_text(APP_LAUNCH_ERR_GENERAL, "internal error");
    finish_prelaunching(prelaunching_item);
    return;
  }
...
void Prelauncher4Base::handle_stages(AppLaunchingItem4BasePtr prelaunching_item) {
  ...
  if (StageHandlerReturn4Base::ERROR == handler_return) {
    LOG_ERROR(MSGID_APPLAUNCH_ERR, 1,
              PMLOGKS("stage", "prelaunching"), "handle_stage: handling fail");
    finish_prelaunching(prelaunching_item);
    return;
...
void Prelauncher4Base::redirect_to_another(AppLaunchingItem4BasePtr prelaunching_item) {
  LOG_INFO(MSGID_APPLAUNCH, 3,
           ...
           PMLOGKS("new_app_id", prelaunching_item->app_id().c_str()),
           PMLOGKS("old_app_id", prelaunching_item->requested_app_id().c_str()),
           "%s->%s", prelaunching_item->requested_app_id().c_str(),
           prelaunching_item->app_id().c_str());
