...
LaunchPointManager::LaunchPointManager()
    : lpm_ready_(false),
      ...
      launch_points_db_data_(pbnjson::Array()),
      db_handler_(nullptr),
      lp_factory_(nullptr),
      ordering_handler_(nullptr) {
}
...
void LaunchPointManager::Init() {
  ...
  ServiceObserver::instance().Add(WEBOS_SERVICE_DB,
      boost::bind(&LaunchPointManager::HandleDbConnected, &LaunchPointManager::instance(), _1));

  AppinstalldSubscriber::instance().SubscribeInstallStatus(
      boost::bind(&LaunchPointManager::OnPackageStatusChanged, this, _1, _2));

  ApplicationManager::instance().signalListAppsChanged.connect(
      boost::bind(&LaunchPointManager::OnListAppsChanged, this, _1, _2, _3));

  ApplicationManager::instance().signalOneAppChanged.connect(
      boost::bind(&LaunchPointManager::OnOneAppChanged, this, _1, _2, _3, _4));
}
...
void LaunchPointManager::SetDbHandler(DbHandlerInterface& db_handler) {
  ...
  if (db_handler_ == nullptr) {
    LOG_ERROR(MSGID_LAUNCH_POINT_ERROR, 3, PMLOGKS("error", "handler_is_null"),
                                           PMLOGKS("which", "db_handler"),
                                           PMLOGKS("where", "SetDbHandler"), "");
    return;
  }
...
void LaunchPointManager::SetOrderingHandler(OrderingHandlerInterface& ordering_handler) {
  ...
  if (ordering_handler_ == nullptr) {
    LOG_ERROR(MSGID_LAUNCH_POINT_ERROR, 3, PMLOGKS("error", "handler_is_null"),
                                           PMLOGKS("which", "ordering_handler"),
                                           PMLOGKS("where", "SetOrderingHandler"), "");
    return;
  }
...
void LaunchPointManager::HandleLpmState(const LPStateInput& input, const pbnjson::JValue& data) {
  ...
  LOG_INFO(MSGID_LAUNCH_POINT, 6, PMLOGKS("status", "handle_lpm_state"),
                                  PMLOGKFV("state_input", "%d", (int)input),
                                  PMLOGKS("load_app", apps_loaded_?"done":"not_ready_yet"),
                                  PMLOGKS("load_db", db_loaded_?"done":"not_ready_yet"),
                                  PMLOGKS("load_ordered_list", ordered_list_ready_?"done":"not_ready_yet"),
                                  PMLOGKS("lpm_ready", lpm_ready_?"ready":"not_ready_yet"), "");
...
void LaunchPointManager::MakeLaunchPointsInOrder() {
  ...
  if (ordering_handler_ == nullptr) {
    LOG_ERROR(MSGID_LAUNCH_POINT_ERROR, 3, PMLOGKS("error", "handler_is_null"),
                                           PMLOGKS("which", "ordering_handler"),
                                           PMLOGKS("where", "MakeLaunchPointsInOrder"), "");
    return;
  }
...
void LaunchPointManager::OnLaunchPointsOrdered(const OrderChangeState& change_state) {
  ...
  LOG_INFO(MSGID_LAUNCH_POINT, 3, PMLOGKS("status", "received_changed_order_list"),
                                  PMLOGKFV("change_state", "%d", (int)change_state),
                                  PMLOGKS("where", "OnLaunchPointsOrdered"), "");
...
void LaunchPointManager::OnListAppsChanged(
  ...
  LOG_INFO(MSGID_LAUNCH_POINT, 3, PMLOGKS("status", "full_list_changed_on_list_apps"),
                                  PMLOGKS("change_reason", JUtil::jsonToString(list_apps_changes_).c_str()),
                                  PMLOGKS("where", "OnListAppsChanged"), "");

  HandleLpmState(LPStateInput::LOAD_APPS, apps);
}

void LaunchPointManager::OnOneAppChanged(
  ...
  LOG_INFO(MSGID_LAUNCH_POINT, 4, PMLOGKS("status", "one_app_changed_on_list_apps"),
                                  PMLOGKS("app_id", app_id.c_str()),
                                  PMLOGKS("change_reason", change.c_str()),
                                  PMLOGKS("where", "OnOneAppChanged"), "");
...
void LaunchPointManager::UpdateApps(const pbnjson::JValue& apps) {
  ...
  for (int i = 0 ; i < apps.arraySize() ; ++i) {
    ...
    if (lp == nullptr)
      LOG_WARNING(MSGID_LAUNCH_POINT_WARNING, 3, PMLOGKS("status", "lp_is_nullptr"),
                                                 PMLOGKS("app_id", app_id.c_str()),
                                                 PMLOGKS("where", "update_apps"), "%s", err_text.c_str());
  }
...
void LaunchPointManager::SyncAppsWithDbData() {
  ...
  LOG_INFO(MSGID_LAUNCH_POINT, 2, PMLOGKS("status", "sync_apps_with_db_data"),
                                  PMLOGKS("action", "start_sync"), "");
  if (db_handler_ == nullptr) {
    LOG_ERROR(MSGID_LAUNCH_POINT_ERROR, 3, PMLOGKS("error", "handler_is_null"),
                                           PMLOGKS("which", "db_handler"),
                                           PMLOGKS("where", "SyncAppsWithDbData"), "");
    return;
  }
...
LaunchPointPtr LaunchPointManager::AddLaunchPoint(const LPMAction action, const pbnjson::JValue& data, std::string& err_text) {
  ...
  switch(action) {
    ...
    case LPMAction::API_ADD:
      ...
      break;
    default:
      LOG_WARNING(MSGID_LAUNCH_POINT_WARNING, 2, PMLOGKS("status", "add_launch_point"),
                                                 PMLOGKS("action", "unknown_request"), "");
  ...
  if ((LPMAction::LOAD_APPS != action) && (LPMAction::DB_SYNC != action)) {
    LOG_INFO(MSGID_LAUNCH_POINT_ACTION, 5, PMLOGKS("status", "add_launch_point"),
                                           PMLOGKS("app_id", app_id.c_str()),
...
LaunchPointPtr LaunchPointManager::UpdateLaunchPoint(const LPMAction action, const pbnjson::JValue& data, std::string& err_text) {
  ...
  if (ordering_handler_ == nullptr) {
    LOG_ERROR(MSGID_LAUNCH_POINT_ERROR, 3, PMLOGKS("error", "handler_is_null"),
                                           PMLOGKS("which", "ordering_handler"),
                                           PMLOGKS("where", "UpdateLaunchPoint"), "");
    err_text = "internal error";
    return nullptr;
  }
  ...
  switch(action) {
    ...
    case LPMAction::APP_UPDATED:
      ...
      break;
    default:
      LOG_WARNING(MSGID_LAUNCH_POINT_WARNING, 2, PMLOGKS("status", "update_launch_point"),
                                                 PMLOGKS("action", "unknown_request"), "");
      err_text = "unknown action";
      return nullptr;
  }
  ...
  switch(action) {
    ...
    case LPMAction::APP_UPDATED:
      ...
      if (updated_lp == nullptr) return nullptr;
  ...
  if (LPMAction::DB_SYNC != action)
    LOG_INFO(MSGID_LAUNCH_POINT_ACTION, 5, PMLOGKS("status", "update_launch_point"),
                                           PMLOGKS("app_id", app_id.c_str()),
                                           PMLOGKFV("lp_type", "%d", (int)lp_by_lp_id->LpType()),
                                           PMLOGKFV("lp_action", "%d", (int)action),
                                           PMLOGKFV("position", "%d", position), "");
  return lp_by_lp_id;
}
...
LaunchPointPtr LaunchPointManager::MoveLaunchPoint(const LPMAction action, const pbnjson::JValue& data, std::string& err_text) {
  ...
  switch(action) {
    case LPMAction::API_MOVE:
      ...
      break;
    default:
      LOG_WARNING(MSGID_LAUNCH_POINT_WARNING, 2, PMLOGKS("status", "move_launch_point"),
                                                 PMLOGKS("action", "unknown_request"), "");
      err_text = "unknown action";
      return nullptr;
  }
  ...
  LOG_INFO(MSGID_LAUNCH_POINT_ACTION, 5, PMLOGKS("status", "move_launch_point"),
...
void LaunchPointManager::RemoveLaunchPoint(const pbnjson::JValue& data, std::string& err_text) {
  ...
  if (ordering_handler_ == nullptr) {
    LOG_ERROR(MSGID_LAUNCH_POINT_ERROR, 3, PMLOGKS("error", "handler_is_null"),
                                           PMLOGKS("which", "ordering_handler"),
                                           PMLOGKS("where", "RemoveLaunchPoint"), "");
    err_text = "internal error";
    return;
  }
  ...
  LOG_INFO(MSGID_LAUNCH_POINT_ACTION, 3, PMLOGKS("status", "remove_launch_point"),
                                         PMLOGKS("app_id", (lp->Id()).c_str()),
                                         PMLOGKFV("lp_type", "%d", static_cast<int>(lp->LpType())), "");
...
void LaunchPointManager::ReplyLpChangeToSubscribers(LaunchPointPtr lp, const std::string& change, int position) {
  ...
  if ((LPType::DEFAULT == lp->LpType()) && !lp->IsVisible()) return;
...
bool LaunchPointManager::IsLastVisibleLp(LaunchPointPtr lp) {
  ...
  for (auto it : launch_point_list_) {
    if ((it->Id() == lp->Id()) &&
        (it->LaunchPointId() != lp->LaunchPointId()) &&
        (it->IsVisible()))
      return false;
  }
  return true;
...
void LaunchPointManager::RemoveAllLaunchPointsByAppId(const std::string& id) {
  ...
  if (ordering_handler_ == nullptr) {
    LOG_ERROR(MSGID_LAUNCH_POINT_ERROR, 3, PMLOGKS("error", "handler_is_null"),
                                           PMLOGKS("which", "ordering_handler"),
                                           PMLOGKS("where", "RemoveAllLaunchPointsByAppId"), "");
    return;
  }
...
void LaunchPointManager::LaunchPointsAsJson(pbnjson::JValue& array) {
  if (ordering_handler_ == nullptr) {
    LOG_ERROR(MSGID_LAUNCH_POINT_ERROR, 3, PMLOGKS("error", "handler_is_null"),
                                           PMLOGKS("which", "ordering_handler"),
                                           PMLOGKS("where", "LaunchPointsAsJson"), "");
    return;
  }
