...
class LaunchPoint
{
 public:
  ...
  void SetLpType(const LPType lp_type) { lp_type_ = lp_type; }
  void SetFolderPath(const std::string& path) { folder_path_ = path; }
  ...
  void SetTileSize(const std::string& tile_size);
  void SetParams(const pbnjson::JValue& params);
  void SetMiniIcon(const std::string& mini_icon) { mini_icon_ = mini_icon; }
  void SetDefaultLp(bool val) { default_ = val; }
  void SetSystemApp(bool val) { system_app_ = val; }
  void SetRemovable(bool val) { removable_ = val; }
  void SetVisible(bool val) { visible_ = val; }
  void SetUnmovable(bool val) { unmovable_ = val; }
  void SetStoredInDb(bool val) { stored_in_db_ = val; }

  LPType LpType() const { return lp_type_; }
  std::string LpTypeAsString() const;
  const std::string& Id() const { return id_; }
  const std::string& LaunchPointId() const { return launch_point_id_; }
  const std::string& Title() const { return title_; }
  const std::string& Icon() const { return icon_; }
  const std::string& BgImage() const { return bg_image_; }
  const pbnjson::JValue& BgImages() const { return bg_images_; }
  const std::string& BgColor() const { return bg_color_; }
  const std::string& ImageForRecents() const { return image_for_recents_; }
  const std::string& IconColor() const { return icon_color_; }
  const std::string& LargeIcon() const { return large_icon_; }
  const std::string& AppDescription() const {return app_description_; }
  const std::string& UserData() const { return user_data_; }
  const std::string& TileSize() const { return tile_size_; }
  const pbnjson::JValue& Params() const { return params_; }
  const std::string& MiniIcon() const { return mini_icon_; }
  bool IsSystemApp() const { return system_app_; }
  bool IsDefault() const { return default_; }
  bool IsRemovable() const { return removable_; }
  bool IsVisible() const { return visible_; }
  bool IsUnmovable() const { return unmovable_; }
  bool IsStoredInDb() const { return stored_in_db_; }
...
{
 public:
  LPDefault(const std::string& id, const std::string& lp_id): LaunchPoint(id, lp_id) {}
...
{
 public:
  LPBookmark(const std::string& id, const std::string& lp_id): LaunchPoint(id, lp_id) {}
