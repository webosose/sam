...
void VirtualAppManager::InstallTmpVirtualAppOnLaunch(const pbnjson::JValue& jmsg, LSMessage* lsmsg) {
  ...
  if (!ValidataHostApp(host_id)) {
    LOG_INFO(MSGID_VIRTUAL_APP_WARNING, 3, PMLOGKS("app_id", app_id.c_str()),
                                           PMLOGKS("host_id", host_id.c_str()),
                                           PMLOGKS("status", "invalid_host_app"), "");
  ...
  if (app_desc) {
    LOG_INFO(MSGID_TMP_VIRTUAL_APP_LAUNCH, 2, PMLOGKS("app_id", app_id.c_str()),
                                              PMLOGKS("status", "just_launch"), "already exist");

    AppLifeManager::instance().launch(AppLaunchRequestType::EXTERNAL_FOR_VIRTUALAPP,
                                      app_id, new_request->jmsg, new_request->lsmsg);
...
bool VirtualAppManager::OnTempPackageReady(LSHandle* lshandle, LSMessage* lsmsg, void* user_data) {
  ...
  if (item == NULL) {
    LOG_ERROR(MSGID_VIRTUAL_APP_WARNING, 2, PMLOGKFV("token", "%d", (int) token),
                                            PMLOGKS("status", "failed_to_get_data_from_async_return"),
  ...
  LOG_INFO(MSGID_TMP_VIRTUAL_APP_LAUNCH, 2, PMLOGKS("app_id", app_id.c_str()),
                                            PMLOGKS("status", "received_async_return"), "token: %d", (int)token);
  ...
  if (jmsg.isNull() || !jmsg.hasKey("returnValue") || !jmsg["returnValue"].asBool()) {
    LOG_ERROR(MSGID_VIRTUAL_APP_WARNING, 2, PMLOGKS("app_id", app_id.c_str()),
                                            PMLOGKS("status", "received_false"),
                                            "return: %s", JUtil::jsonToString(jmsg).c_str());
  ...
  if (current_app_desc && current_app_desc->getTypeByDir() == AppTypeByDir::Alias) {
    ...
    LOG_INFO(MSGID_TMP_VIRTUAL_APP_LAUNCH, 2, PMLOGKS("app_id", app_id.c_str()),
                                              PMLOGKS("status", "remove_existing_app_desc"), "");
...
void VirtualAppManager::CreateVirtualApp(VirtualAppRequestPtr request) {
  ...
  if (!CreateVirtualAppPackage(app_id, host_id, request->jmsg["appInfo"], request->jmsg["params"], app_type, error_text)) {
    LOG_INFO(MSGID_VIRTUAL_APP_WARNING, 3, PMLOGKS("app_id", app_id.c_str()),
                                           PMLOGKS("host_id", host_id.c_str()),
                                           PMLOGKS("status", "failed_to_create_virtual_app_package"), "");
  ...
  if (!ret_val) {
    LOG_INFO(MSGID_VIRTUAL_APP_WARNING, 3, PMLOGKS("app_id", app_id.c_str()),
                                            PMLOGKS("host_id", host_id.c_str()),
                                            PMLOGKS("status", "failed_to_request_creating_ls_config"), "");
    request->error_code = ERROR_CODE_COMMON;
  ...
  LOG_INFO(MSGID_ADD_VIRTUAL_APP, 2, PMLOGKS("app_id", app_id.c_str()),
                                     PMLOGKS("status", "requested_creating_ls_config"),
                                     "token: %d", (int) request->async_job_token);
}
...
bool VirtualAppManager::OnPackageReady(LSHandle* lshandle, LSMessage* lsmsg, void* user_data) {
  ...
  LOG_INFO(MSGID_ADD_VIRTUAL_APP, 2, PMLOGKS("app_id", app_id.c_str()),
                                     PMLOGKS("status", "received_async_return"),
                                     "token: %d", (int)token);
...
VirtualAppRequestPtr VirtualAppManager::MakeVirtualAppRequest(VirtualAppType app_type, const pbnjson::JValue& jmsg, LSMessage* lsmsg) {
  ...
  if(new_request == NULL) return NULL;
...
VirtualAppRequestPtr VirtualAppManager::GetWorkItemByToken(LSMessageToken token) {
  auto it = std::find_if(work_items_.begin(), work_items_.end(),
                        [&token](VirtualAppRequestPtr request){return (request->async_job_token == token);});
  if (it == work_items_.end()) return NULL;
  return (*it);
}
...
void VirtualAppManager::ReplyForRequest(LSMessage* lsmsg,
    int error_code, const std::string& error_text) {
  if (lsmsg == NULL) return;
  ...
  if (!LSMessageRespond(lsmsg, JUtil::jsonToString(payload).c_str(), &lserror)) {
    LOG_ERROR(MSGID_LSCALL_ERR, 2, PMLOGKS("type", "respond"),
...
bool VirtualAppManager::ValidataHostApp(const std::string& host_id) {
  ...
  if (!SettingsImpl::instance().is_in_host_apps_list(host_id)) {
    LOG_INFO(MSGID_VIRTUAL_APP_WARNING, 2, PMLOGKS("host_id", host_id.c_str()),
                                           PMLOGKS("status", "not_in_host_apps_list"), "");
    return false;
  }
...
bool VirtualAppManager::CreateVirtualAppPackage(
        const std::string& app_id,
        const std::string& host_id,
        const pbnjson::JValue& app_info,
        const pbnjson::JValue& launch_params,
        VirtualAppType app_type,
        std::string& error_text) {

  if (app_id.find("com.palm.") == 0 ||
      app_id.find("com.webos.") == 0 ||
      app_id.find("com.lge.") == 0 ) {
    LOG_INFO(MSGID_VIRTUAL_APP_WARNING, 2, PMLOGKS("app_id", app_id.c_str()),
                                           PMLOGKS("status", "cannot_use_privileged_app_id"), "");
  ...
  if (!host_desc) {
    LOG_WARNING(MSGID_VIRTUAL_APP_WARNING, 3, PMLOGKS("app_id", app_id.c_str()),
                                              PMLOGKS("host_id", host_id.c_str()),
                                              PMLOGKS("status", "host_app_does_not_exist"), "");
  ...
  LOG_INFO(MSGID_VIRTUAL_APP_INFO_CREATED, 3, PMLOGKS("app_id", app_id.c_str()),
                                              PMLOGKS("host_id", host_id.c_str()),
                                              PMLOGKFV("app_type", "%d", (int)app_type), "");
  ...
  if (0 != g_mkdir_with_parents(app_root_path.c_str(), 0700)) {
    LOG_ERROR(MSGID_VIRTUAL_APP_WARNING, 3, PMLOGKS("app_id", app_id.c_str()),
                                            PMLOGKS("app_root_path", app_root_path.c_str()),
                                            PMLOGKS("status", "failed_to_create_base_app_directory"), "");
  ...
  if (!g_file_set_contents(app_info_path.c_str(), str_app_info.c_str(), str_app_info.length(), NULL)) {
    LOG_ERROR(MSGID_VIRTUAL_APP_WARNING, 3, PMLOGKS("app_id", app_id.c_str()),
                                            PMLOGKS("app_info_path", app_info_path.c_str()),
                                            PMLOGKS("status", "failed_to_write_app_info"), "");
    error_text = VIRTUALAPP_ERR_TXT_CREATING_APP_INFO_FAIL;
    return false;
  }
...
bool VirtualAppManager::CreateLSConfig(const std::string& app_id,
    const std::string& device_id, LSFilterFunc cb_func, LSMessageToken& msg_token) {
  ...
  if (!LSCallOneReply(AppMgrService::instance().ServiceHandle(),
                      "luna://com.webos.appInstallService/createLSConfigFiles",
                      payload.c_str(),
                      cb_func, NULL, &msg_token, &lserror)) {
    LOG_ERROR(MSGID_LSCALL_ERR, 3, PMLOGKS("type", "lscallonereply"),
                                   PMLOGKS("service_name", "com.webos.appInstallService"),
                                   PMLOGKS("request", "create_ls_config"), "err: %s", lserror.message);
    return false;
  }
  return true;
}

bool VirtualAppManager::RemoveLSConfig(const std::string& app_id,
    const std::string& device_id, LSFilterFunc cb_func, LSMessageToken* msg_token) {
  std::string payload = std::string("{\"id\":\"") + app_id + std::string("\",\"target\":{\"deviceId\":\"") + device_id + std::string("\"}}");
  LSErrorSafe lserror;
  if (!LSCallOneReply(AppMgrService::instance().ServiceHandle(),
                      "luna://com.webos.appInstallService/removeLSConfigFiles",
                      payload.c_str(),
                      cb_func, NULL, msg_token, &lserror)) {
    LOG_ERROR(MSGID_LSCALL_ERR, 3, PMLOGKS("type", "lscallonereply"),
                                   PMLOGKS("service_name", "com.webos.appInstallService"),
                                   PMLOGKS("request", "remove_ls_config"), "err: %s", lserror.message);
    return false;
  }
  return true;
}
...
void VirtualAppManager::RemoveVirtualAppPackage(const std::string& app_id, VirtualAppType app_type) {
  ...
  if (!removeDir(path)) {
    LOG_ERROR(MSGID_VIRTUAL_APP_WARNING, 4, PMLOGKS("app_id", app_id.c_str()),
                                            PMLOGKFV("app_type", "%d", (int)app_type),
                                            PMLOGKS("path", path.c_str()),
                                            PMLOGKS("status", "failed_to_remove_dir"), "");
    return;
  }

  LOG_INFO(MSGID_UNINSTALL_APP, 2, PMLOGKS("app_id", app_id.c_str()),
                                   PMLOGKS("app_type", app_type == VirtualAppType::REGULAR ? "virtual_app":"tmp_virtual_app"),
                                   "path: %s", path.c_str());
