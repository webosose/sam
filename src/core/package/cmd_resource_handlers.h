...
public:
    ...
    int32_t setIndex(int32_t newindex)
    {
        int32_t t = m_index;
        m_index = newindex;
        return t;
...
public:
    ResourceHandler(const std::string& ext, const std::string& contentType, const std::string& appId, bool stream = false, int32_t index = -1);
    ResourceHandler(const std::string& ext, const std::string& contentType, const std::string& appId, bool stream, const std::string& handler_tag, int32_t index = -1);

    ~ResourceHandler()
    {
    }
    ResourceHandler() :
            m_stream(false), m_valid(false), m_index(0)
    {
    }
