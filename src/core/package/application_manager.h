...
class ApplicationManager : public Singleton<ApplicationManager> {
 public:
  ...
  AppScanner& appScanner() { return app_scanner_; }
  ...
  // Event Signals
  boost::signals2::signal<void (const pbnjson::JValue&, const std::vector<std::string>&,
                                bool dev)> signalListAppsChanged;
  boost::signals2::signal<void (const pbnjson::JValue&, const std::string&,
                                const std::string&, bool dev)> signalOneAppChanged;
 ...
 private:
  ...
  void Scan();
  void Clear();
  void AddApp(const std::string& app_id, AppDescPtr new_desc,
              AppStatusChangeEvent event = AppStatusChangeEvent::APP_INSTALLED);
  void RemoveApp(const std::string& app_id, bool rescan = true,
                 AppStatusChangeEvent event = AppStatusChangeEvent::APP_UNINSTALLED);
