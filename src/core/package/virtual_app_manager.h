...
class VirtualAppManager : public Singleton<VirtualAppManager> {
 ...
 private:
  ...
  VirtualAppRequestPtr MakeVirtualAppRequest(VirtualAppType app_type,
        const pbnjson::JValue& jmsg, LSMessage* lsmsg);
  ...
  bool CreateVirtualAppPackage(
        ...
        VirtualAppType app_type,
        std::string& error_text);
  bool CreateLSConfig(const std::string& app_id,
        const std::string& device_id, LSFilterFunc cb_func, LSMessageToken& msg_token);

  bool RemoveLSConfig(const std::string& app_id,
        const std::string& device_id, LSFilterFunc cb_func, LSMessageToken* msg_token);
