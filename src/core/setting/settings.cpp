...
Settings::Settings()
    : appMgrPreferenceDir( kAppMgrPreferenceDir ),
      deletedSystemAppListPath( kDeletedSystemAppListPath ),
      devModePath( "/var/luna/preferences/devmode_enabled" ),
      localeInfoPath( kLocaleInfoFile ),
      schemaPath( kSchemaPath ),
      respawnedPath( "/tmp/sam-respawned" ),
      jailModePath( "/var/luna/preferences/jailer_disabled" ),
      log_path_(kLogBasePath),
      isRespawned( false ),
      isDevMode( false ),
      isJailMode( false ),
      isRestLoaded( false ),
      ...
      appInstallBase( kAppInstallBase ),
      appInstallRelative( "usr/palm/applications" ),
      devAppsBasePath( "/media/developer/apps" ),
      tempAliasAppBasePath( "/tmp/alias/apps" ),
      aliasAppBasePath( "/media/alias/apps" ),
...
void Settings::LoadConfigOnRWFilesystemReady() {
  ...
  if (0 ==access(devModePath.c_str(), F_OK)) {
    ...
    for (auto it: devAppsPaths) {
      AddBaseAppDirPath(it, AppTypeByDir::Dev);
    }
  }
  ...
  if (0 ==access(respawnedPath.c_str(), F_OK)) {
    ...
    if (!writeFile(respawnedPath.c_str(), "")) {
      LOG_WARNING(MSGID_SETTINGS_ERR, 1, PMLOGKS("respawnedPath", respawnedPath.c_str()),
                                         "cannot create respawned file");
    }
  }

  LOG_INFO(MSGID_SAM_LOADING_SEQ, 2, PMLOGKS("status", "analyze_loading_reason"),
                                     PMLOGKS("loading_status", (isRespawned ? "respawned" : "fresh_start")), "");

  if (0 != g_mkdir_with_parents(appMgrPreferenceDir.c_str(), 0700)) {
    LOG_WARNING(MSGID_SETTINGS_ERR, 1, PMLOGKS("PreferencesDir", appMgrPreferenceDir.c_str()),
...
bool Settings::LoadStaticConfig(const char* filePath) {
  ...
  if (root["ApplicationPaths"].isArray()) {
    ...
    for (int i = 0; i < arraySize ; i++) {
      ...
      if("system_builtin" == type)
          typeByDir = AppTypeByDir::System_BuiltIn;
      else if ("system_updatable" == type)
          typeByDir = AppTypeByDir::System_Updatable;
      else if("store" == type)
          typeByDir = AppTypeByDir::Store;
      else if("dev" == type) {
          devAppsPaths.push_back(path);
          continue;
      }
      else if("alias" == type)
          typeByDir = AppTypeByDir::Alias;
      else if("tmp_alias" == type)
          typeByDir = AppTypeByDir::TempAlias;
      else
          continue;
  ...
  if (root.hasKey("FullscreenWindowType") && root["FullscreenWindowType"].isArray()) {
    ...
    for (int i = 0 ; i < array_size ; ++i) {
      std::string value;
      if (root["FullscreenWindowType"][i].asString(value) != CONV_OK) continue;
  ...
  if (root["NoJailApps"].isArray()) {
    ...
    for (int i = 0; i < arraySize; ++i) {
      std::string str;
      if ( appList[i].asString(str) != CONV_OK ) continue;
  ...
  if (root["ReservedResource"].isObject()) {
    ...
    if (reservedRoot["mime"].isArray()) {
      ...
      for (int i = 0; i < arraySize; ++i) {
        if ( subList[i].asString(str) != CONV_OK ) continue;
...
void Settings::setKeepAliveApps(const pbnjson::JValue& apps) {
  if (!apps.isArray()) return;
  keepAliveApps.clear();

  LOG_INFO(MSGID_CONFIGD_INIT, 1, PMLOGKS("CONFIG_TYPE", "keep_alive_apps"),
                                  "%s", JUtil::jsonToString(apps).c_str());
  ...
  for (int i = 0 ; i < apps_num ; ++i) {
    ...
    LOG_INFO(MSGID_CONFIGD_INIT, 2, PMLOGKS("CONFIG_TYPE", "keep_alive_apps"),
...
void Settings::set_support_qml_booster(bool value) {
  ...
  LOG_INFO(MSGID_CONFIGD_INIT, 2, PMLOGKS("CONFIG_TYPE", "support_qml_booster"),
...
void Settings::setAssetFallbackKeys(const pbnjson::JValue& keys_arr) {
  ...
  LOG_INFO(MSGID_CONFIGD_INIT, 1, PMLOGKS("CONFIG_TYPE", "asset_fallback_precedence"),
                                  "%s", JUtil::jsonToString(keys_arr).c_str());

  if (!keys_arr.isArray()) return;
  ...
  for (int i = 0 ; i < keys_num ; ++i) {
    ...
    LOG_INFO(MSGID_CONFIGD_INIT, 2, PMLOGKS("CONFIG_TYPE", "asset_fallback_precedence"),
...
void Settings::AddBootTimeApp(const std::string& app_id) {
  ...
  if (it != boot_time_apps_.end()) return;
...
void Settings::setSystemAppAsRemoved(const std::string& appId)
{
    ...
    if(!g_file_set_contents(deletedSystemAppListPath.c_str(),
                    deletedList.c_str(), deletedList.length(), NULL))
    {
        LOG_WARNING(MSGID_FAIL_WRITING_DELETEDLIST, 2, PMLOGKS("CONTENT", deletedList.c_str()),
                                                       PMLOGKS("PATH", deletedSystemAppListPath.c_str()), "");
    }
}
...
bool Settings::isDeletedSystemApp(const std::string& appId) const
{
    if (m_deletedSystemApps.isNull() ||
        !m_deletedSystemApps.isObject() ||
        !m_deletedSystemApps.hasKey(DELETED_LIST_KEY) ||
        !m_deletedSystemApps[DELETED_LIST_KEY].isArray()) return false;
    int arraySize = m_deletedSystemApps[DELETED_LIST_KEY].arraySize();
    if(arraySize < 1)
        return false;
...
void Settings::SetLifeCycleReason(const pbnjson::JValue& data) {
  if (data.hasKey("launchReason") &&
      data["launchReason"].isObject() && data["launchReason"].objectSize() > 0) {

    for(auto it : data["launchReason"].children()) {
      if(!it.second.isArray() || it.second.arraySize() < 1) continue;
  ...
  if (data.hasKey("closeReason") &&
      data["closeReason"].isObject() && data["closeReason"].objectSize() > 0) {
