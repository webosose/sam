...
WebAppLifeHandler::WebAppLifeHandler()
    ...
    , m_running_list(pbnjson::Array())
{
    g_this = this;

    ServiceObserver::instance().Add(WEBOS_SERVICE_WAM,
        std::bind(&WebAppLifeHandler::on_wam_service_status_changed, this, std::placeholders::_1));
...
void WebAppLifeHandler::launch(AppLaunchingItemPtr item)
{
    ...
    if(app_desc == NULL)
    {
        LOG_ERROR(MSGID_APPLAUNCH_ERR, 3, PMLOGKS("app_id", item->app_id().c_str()), PMLOGKS("reason", "null_description"), PMLOGKS("where", "webapp_launch"), "");
    ...
    payload.put("launchingAppId", item->caller_id());
    payload.put("launchingProcId", item->caller_pid());
    payload.put("keepAlive", item->keep_alive());
    ...
    add_loading_app(item->app_id());

    if (item->preload().empty())
      signal_app_life_status_changed(item->app_id(), item->uid(), RuntimeStatus::LAUNCHING);
    else
      signal_app_life_status_changed(item->app_id(), item->uid(), RuntimeStatus::PRELOADING);

    double current_time = get_current_time();
    double elapsed_time = current_time - item->launch_start_time();
    LOG_INFO(MSGID_APP_LAUNCHED, 5, PMLOGKS("app_id", item->app_id().c_str()),
                                    PMLOGKS("type", "web"),
                                    PMLOGKS("pid", ""),
                                    PMLOGKFV("start_time", "%f", item->launch_start_time()),
                                    PMLOGKFV("collapse_time", "%f", elapsed_time), "");

    item->set_return_token(token);
...
bool WebAppLifeHandler::cb_return_for_launch_request(LSHandle* handle, LSMessage* lsmsg, void* user_data)
{
    ...
    std::string app_id = jmsg.hasKey("appId") && jmsg["appId"].isString() ? jmsg["appId"].asString() : item->app_id();
    ...
    {
        LOG_ERROR(MSGID_APPLAUNCH_ERR, 2, PMLOGKS("app_type", "webapp")
                    , PMLOGKS("app_id", app_id.c_str()), "err: %s", JUtil::jsonToString(jmsg).c_str());

        if(app_id.empty())
            LOG_CRITICAL(MSGID_LIFE_STATUS_ERR, 2, PMLOGKS("reason", "cannot_handle_life_status"), PMLOGKS("where", "wam_launch_cb_return"), "");

        item->set_err_code_text(APP_LAUNCH_ERR_GENERAL, "WebAppMgr's launchApp is failed");
...
void WebAppLifeHandler::pause(const std::string& app_id, const pbnjson::JValue& params, std::string& err_text, bool send_life_event) {
  ...
  if (!LSCallOneReply(AppMgrService::instance().ServiceHandle(),
              ...
              JUtil::jsonToString(payload).c_str(),
              cb_return_for_pause_request, this, NULL, &lserror)) {
    LOG_ERROR(MSGID_LSCALL_ERR, 3, PMLOGKS("type", "lscallonereply"),
                                   PMLOGJSON("payload", JUtil::jsonToString(payload).c_str()),
  ...
  if (send_life_event) signal_app_life_status_changed(app_id, "", RuntimeStatus::PAUSING);
}
...
bool WebAppLifeHandler::cb_return_for_pause_request(LSHandle* handle, LSMessage* lsmsg, void* user_data) {
  ...
  pbnjson::JValue jmsg = JUtil::parse(LSMessageGetPayload(lsmsg), std::string(""));
  if (jmsg.isNull()) {
    LOG_ERROR(MSGID_APPPAUSE_ERR, 2, PMLOGKS("reason", "parsing_fail"),
  ...
  if(jmsg["returnValue"].asBool()) {
    std::string app_id = jmsg["appId"].asString();
    LOG_INFO(MSGID_APPPAUSE, 2, PMLOGKS("app_id", app_id.c_str()),
                                PMLOGKS("status", "received_pause_return_from_wam"), "");
  } else {
    std::string err_text = jmsg["errorText"].asString();
    LOG_ERROR(MSGID_APPPAUSE, 2, PMLOGKS("reason", "return_false"),
...
void WebAppLifeHandler::on_wam_service_status_changed(bool connection)
{
    ...
    else
    {
        ...
        if(0 != m_wam_subscription_token)
        {
            ...
            {
                LOG_ERROR(MSGID_LSCALL_ERR, 3, PMLOGKS("type", "lscallcancel"),
                                               PMLOGKS("payload", ""),
                                               PMLOGKS("where", "wam_listRunningApps"), "err: %s", lserror.message);
            }
            m_wam_subscription_token = 0;
        }
...
{
    if(AppMgrService::instance().IsServiceReady() == false ||
       ServiceObserver::instance().IsConnected(WEBOS_SERVICE_WAM) == false ||
       m_wam_subscription_token != 0)
    {
        return;
    }
    ...
    if(!LSCall(AppMgrService::instance().ServiceHandle(),
            "luna://com.palm.webappmanager/listRunningApps",
            "{\"includeSysApps\":true,\"subscribe\":true}",
            cb_wam_subscription_runninglist,
            this, &m_wam_subscription_token, &lserror))
    {
...
bool WebAppLifeHandler::cb_wam_subscription_runninglist(LSHandle* handle, LSMessage* lsmsg, void* user_data)
{
    ...
    {
        LOG_ERROR(MSGID_WAM_RUNNING_ERR, 3, PMLOGKS("reason", "invalid_running")
...
void WebAppLifeHandler::handle_running_list_change(const pbnjson::JValue& new_list)
{
    ...
    for(int i = 0 ; i < old_list_len ; ++i)
    {
        ...
        for(int j = 0 ; j < new_list_len ; ++j)
        {
            ...
            {
                if(new_list[j]["webprocessid"].asString() == old_webprocid) is_removed = false;
        ...
        if(is_removed)
        {
            LOG_INFO(MSGID_APP_CLOSED, 3, PMLOGKS("app_id", old_app_id.c_str()),
                                          PMLOGKS("type", "web"),
                                          PMLOGKS("pid", ""), "");

            LOG_INFO(MSGID_WAM_RUNNING, 5, PMLOGKS("app_id", old_app_id.c_str()),
    ...
    for(int i = 0 ; i < new_list_len ; ++i)
    {
        ...
        for(int j = 0 ; j < old_list_len ; ++j)
        {
            ...
            {
                if(m_running_list[j]["webprocessid"].asString() == new_webprocid) is_added = false;
                else is_changed = true;
        ...
        {
            LOG_INFO(MSGID_WAM_RUNNING_ERR, 5, PMLOGKS("app_id", new_app_id.c_str()),
        ...
        if(is_added)
        {
            LOG_INFO(MSGID_WAM_RUNNING, 5, PMLOGKS("app_id", new_app_id.c_str()),
...
void WebAppLifeHandler::remove_item_from_lscall_request_list(const std::string& uid)
{
    ...
    LOG_INFO(MSGID_APPLAUNCH, 2, PMLOGKS("app_id", (*it)->app_id().c_str()), PMLOGKS("uid", uid.c_str()), "removed from checking queue");
