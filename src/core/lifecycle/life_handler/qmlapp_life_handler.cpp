...
void QmlAppLifeHandler::launch(AppLaunchingItemPtr item)
{
    ...
    if(app_desc == NULL)
    {
        LOG_ERROR(MSGID_APPLAUNCH_ERR, 3, PMLOGKS("app_id", item->app_id().c_str()), PMLOGKS("reason", "null_description"), PMLOGKS("where", "qmlapp_launch"), "");
...
bool QmlAppLifeHandler::cb_return_booster_launch(LSHandle* handle, LSMessage* lsmsg, void* user_data)
{
    ...
    if (jmsg.isNull())
    {
        ...
        item->set_err_code_text(APP_LAUNCH_ERR_GENERAL, "booster error");
    ...
    if(!jmsg.hasKey("returnValue") || jmsg["returnValue"].asBool() == false)
    {
        LOG_ERROR(MSGID_APPLAUNCH_ERR, 2, PMLOGKS("app_id", item->app_id().c_str()),
                                          ...
                                          "invalid_booster_return err: %s", JUtil::jsonToString(jmsg).c_str());
    ...
    if(app_id.empty())
    {
        LOG_WARNING(MSGID_APPLAUNCH, 2, PMLOGKS("status", "received_empty_app_id"),
                                        PMLOGKS("restoring_app_id", item->app_id().c_str()),
                                        "now restore app_id from sam info");
        app_id = item->app_id();
    }
    ...
    {
        LOG_ERROR(MSGID_APPLAUNCH_ERR, 2, PMLOGKS("app_id", item->app_id().c_str()),
                                          PMLOGKS("reason", "received_empty_pid"), "invalid_booster_return err: %s", JUtil::jsonToString(jmsg).c_str());
...
void QmlAppLifeHandler::pause(const std::string& app_id, const pbnjson::JValue& params,
...
bool QmlAppLifeHandler::qml_process_watcher(LSHandle* handle, LSMessage* lsmsg, void* user_data)
{
    ...
    LOG_INFO(MSGID_APP_CLOSED, 3, PMLOGKS("app_id", app_id.c_str()),
                                  PMLOGKS("type", "qml"),
