...
Logger::Logger() : file_no_(1), log_no_(1), max_log_file_(10), max_log_per_file_(10000) {
}
...
void Logger::Init(const std::string& log_path, uint32_t max_log_file, uint32_t max_log_per_file) {
  log_path_ = log_path;
  if (max_log_file_ > 0) max_log_file_ = max_log_file;
  if (max_log_per_file_ > 0) max_log_per_file_ = max_log_per_file;
  OpenFile();
}
...
void Logger::WriteLog(const std::string& data) {
  ...
  if (log_no_ > max_log_per_file_) OpenFile();
  if (!fs_.is_open()) return;
...
void LifeCycleLogger::Init() {
  ...
  AppLifeManager::instance().signal_lifecycle_event.connect(
      boost::bind(&LifeCycleLogger::LifeEventWatcher, this, _1));
  AppLifeManager::instance().signal_app_life_status_changed.connect(
      boost::bind(&LifeCycleLogger::LifeStatusWatcher, this, _1, _2));
}
