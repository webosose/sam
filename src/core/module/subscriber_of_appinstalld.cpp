...
void AppinstalldSubscriber::Init() {
  ServiceObserver::instance().Add(WEBOS_SERVICE_APPINSTALL,
      std::bind(&AppinstalldSubscriber::OnInstallServiceStatusChanged, this, std::placeholders::_1));
  ServiceObserver::instance().Add(WEBOS_SERVICE_APPUPDATE,
      std::bind(&AppinstalldSubscriber::OnUpdateServiceStatusChanged, this, std::placeholders::_1));
}

boost::signals2::connection AppinstalldSubscriber::SubscribeInstallStatus(
    boost::function<void(const std::string&, const PackageStatus&)> func) {
  return notify_install_status_.connect(func);
}

boost::signals2::connection AppinstalldSubscriber::SubscribeUpdateInfo(
    boost::function<void(const pbnjson::JValue&)> func) {
...
void AppinstalldSubscriber::RequestInstallStatus() {
  if (token_install_status_ != 0) return;
  ...
  if (!LSCall(AppMgrService::instance().ServiceHandle(),
      ...
      OnInstallStatusCallback, this, &token_install_status_, &lserror)) {
...
bool AppinstalldSubscriber::OnInstallStatusCallback(LSHandle* handle, LSMessage* lsmsg, void* user_data) {
  ...
  if (!jmsg["details"].isObject()) return false;
  ...
  if (PackageStatus::Uninstalled == package_status &&
      jmsg["details"].hasKey("type") && jmsg["details"]["type"].isString() &&
      jmsg["details"]["type"].asString() == "initialize") {
    package_status = PackageStatus::ResetToDefault;
  }

  app_id = package_id.empty() ? id : package_id;

  LOG_INFO(MSGID_PACKAGE_STATUS, 2, PMLOGKS("app_id", app_id.c_str()),
...
void AppinstalldSubscriber::RequestUpdateInfo() {
  if (token_update_info_ != 0) return;
  ...
  if (!LSCall(AppMgrService::instance().ServiceHandle(),
      method.c_str(), "{\"subscribe\":true}",
      OnUpdateInfoCallback, this, &token_update_info_, &lserror)) {
    LOG_ERROR(MSGID_LSCALL_ERR, 3, PMLOGKS("type", "lscall"),
                                   PMLOGJSON("payload", "{\"subscribe\":true}"),
                                   PMLOGKS("where", __FUNCTION__), "err: %s", lserror.message);
  }
