...
BootdSubscriber::BootdSubscriber() : token_boot_status_(0) {
}
...
void BootdSubscriber::Init() {
  ServiceObserver::instance().Add(WEBOS_SERVICE_BOOTMGR,
      std::bind(&BootdSubscriber::OnServerStatusChanged, this, std::placeholders::_1));
}
...
void BootdSubscriber::RequestBootStatus() {
  if(token_boot_status_ != 0) return;
  ...
  if(!LSCall(AppMgrService::instance().ServiceHandle(),
      ...
      OnBootStatusCallback, this, &token_boot_status_, &lserror)) {
